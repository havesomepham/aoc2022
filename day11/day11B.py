...
# Using number theory, taking the item's value mod lcm(all divisibility test numbers) does not change its originial divisilbity by each of the divisiblity test numebrs in each of the monkeys
big_mod = 1
...
for i in range(10_000):
    for monkey in monkeys:
        while (monkey.items):  # TODO check
            monkey.inspections += 1
            monkey.items[0] = monkey.execute_operation(
                monkey.items[0]) % big_mod
